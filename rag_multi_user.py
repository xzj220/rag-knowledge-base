    ...
    if not is_railway:
        ...
        def start_bore():
            try:
                ...
                if not os.path.exists(bore_exe):
                    return None
                subprocess.Popen([bore_exe, "local", str(port), "--to", "bore.pub"],
                                 stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                time.sleep(3)
                return "http://bore.pub"
            except:
                return None
        public_url = start_ngrok() if (NGROK_TOKEN or TUNNEL_MODE == "ngrok") else (
            start_bore() if TUNNEL_MODE == "bore" else None
        )
